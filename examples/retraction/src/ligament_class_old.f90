...
module ligament_class
   ...
   type :: ligament
      ...
      type(partmesh) :: pmesh_lpt,pmesh    !< For spray conversion
...
contains
   ...
   subroutine init(this)
      ...
      create_and_initialize_vof: block
         use vfs_class, only: VFlo,VFhi,elvira,r2pnet,flux,remap,r2p
         ...
         ! Create a VOF solver
         call this%vf%initialize(cfg=this%cfg,reconstruction_method=r2p,transport_method=flux,name='VOF')
      ...
      create_smesh: block
         ...
         this%smesh%varname(7)='id_ccl_edge'
         this%smesh%varname(8)='id_ccl_ligament'
         this%smesh%varname(9)='id_ccl'
         this%smesh%varname(10)='lig_ind'
         this%smesh%varname(11)='norm_abs'
         this%smesh%varname(12)='norm_sig'
               ...
               do i=this%vf%cfg%imin_,this%vf%cfg%imax_
                  do nplane=1,getNumberOfPlanes(this%vf%liquid_gas_interface(i,j,k))
                     if (getNumberOfVertices(this%vf%interface_polygon(nplane,i,j,k)).gt.0) then
                        ...
                        this%smesh%var(6,np)=this%bu%film_type(i,j,k)!real(this%bu%film_type(i,j,k),WP)
                        this%smesh%var(7,np)=real(this%bu%ccl_edge%id(i,j,k),WP)
                        ...
                        this%smesh%var(11,np)=this%vf%norm_abs(i,j,k)
                        this%smesh%var(12,np)=this%vf%norm_sig(i,j,k)
                     end if
                  end do
               end do
            end do
      ...
      create_pmesh_lpt: block
         ...
         this%pmesh_lpt=partmesh(nvar=2,nvec=1,name='lpt')
         this%pmesh_lpt%varname(1)='diameter'
         this%pmesh_lpt%varname(2)='id'
         this%pmesh_lpt%vecname(1)='velocity'
      ...
      create_pmesh: block
         ...
         this%pmesh=partmesh(nvar=1,nvec=1,name='stokeslet')
         ! this%pmesh%varname(1)='id'
         this%pmesh%varname(1)='fcoeff'
         this%pmesh%vecname(1)='nedge'
      ...
      create_ensight: block
         ...
         call this%ens_out%add_particle('spray',this%pmesh_lpt)
         call this%ens_out%add_particle('stokeslet',this%pmesh)
         ! Output to ensight
         if (this%ens_evt%occurs()) call this%ens_out%write_data(this%time%t)
      end block create_ensight
      
      ! if (this%cfg%amRoot) print *, "Here2"
      ...
      write_data_info: block
         integer :: iunit
         iunit =  1
         if(this%cfg%amRoot) then 
            open(unit=iunit,file='VFint.csv',status='unknown')
            write(iunit,'(a12,3x,a12,3x,a12,3x,a12,3x,a12,3x,a12)') 'Time','VFint','ParticleInt','Nedges','Nparticles'
         end if
      end block write_data_info
   ...
   subroutine step(this)
      ...
      iunit =  1
      if(this%cfg%amRoot) then
         open(unit=iunit,file='VFint.csv',position="append",status='old')
         write(iunit,'(1x,f24.16,1x,f24.16,1x,f24.16,1x,f24.16,1x,f24.16)') this%time%t,this%vf%VFint,this%bu%vol_convert,this%ss%np*1.0_WP, this%lp%np*1.0_WP
         close(iunit)
      end if
      ! Increment time
      call this%fs%get_cfl(this%time%dt,this%time%cfl)
                  ...
                  do i=this%vf%cfg%imin_,this%vf%cfg%imax_
                     do nplane=1,getNumberOfPlanes(this%vf%liquid_gas_interface(i,j,k))
                        if (getNumberOfVertices(this%vf%interface_polygon(nplane,i,j,k)).gt.0) then
                           ...
                           this%smesh%var(4,np)=this%vf%thin_sensor(i,j,k)
                           this%smesh%var(5,np)=this%vf%thickness  (i,j,k)
                           this%smesh%var(6,np)=this%bu%film_type(i,j,k)
                           this%smesh%var(7,np)=real(this%bu%ccl_edge%id(i,j,k),WP)
                           this%smesh%var(8,np)=real(this%vf%ccl%id(i,j,k),WP)
                           this%smesh%var(9,np)=real(this%bu%ccl%id(i,j,k),WP)
                           this%smesh%var(10,np)=real(this%vf%lig_ind(i,j,k),WP)
