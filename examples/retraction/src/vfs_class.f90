...
contains
   ...
   subroutine initialize(this,cfg,reconstruction_method,transport_method,name)
      ...
      case (r2p,r2pnet,r2plig,r2pnetlig)
         ...
         allocate(this%norm_abs(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%norm_abs=0.0_WP
         allocate(this%norm_sig(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%norm_sig=0.0_WP
         ...
         ! Only used for r2plig and r2pnetlig
   ...
   subroutine advance(this,dt,U,V,W,div)
      implicit none
      class(vfs), intent(inout) :: this
      ...
      select case (this%transport_method)
      case (flux)
         call this%transport_flux(dt,U,V,W,div)
      case (flux_storage)
   ...
   subroutine transport_flux(this,dt,U,V,W,div)
      implicit none
      class(vfs), intent(inout) :: this
      real(WP), intent(inout) :: dt  !< Timestep size over which to advance
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: U     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: V     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: W     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
      real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: div
      integer :: i,j,k,index
      real(IRL_double), dimension(3,9) :: face
      ...
      real(WP) :: vol_now,crude_VF,temp_VF
   ...
   subroutine build_r2p(this)
      ...
      this%norm_abs =0.0_WP
      this%norm_sig =0.0_WP
      this%norm_pos =0.0_WP
      this%norm_neg =0.0_WP

      ! Get various measurements of the new metric
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ! Skip wall/bcond/full cells
      ...
      !             size_adj = size_adj + getSize(this%triangle_moments_storage(ii,jj,kk))
      !          end do; end do; end do
      !          size_loc = getSize(this%triangle_moments_storage(i,j,k))
      !          allocate(normals_adj(1:size_adj,1:3));normals_adj = 0.0_WP
      !          allocate(area_adj   (1:size_adj));    area_adj    = 0.0_WP
      !          allocate(norm_pos_loc(1:size_loc));   norm_pos_loc = 0.0_WP
      !          allocate(norm_neg_loc(1:size_loc));   norm_neg_loc = 0.0_WP
      !          size_adj=0

      !          ! store cell i,j,k info first
      !          do ind=0,size_loc-1
      ...
      !             normals_adj(size_adj,:)=normalize(getNormal(volume_moments_and_normal))
      !          end do
      !          ! store the rest of the polygons later
      !          do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
      !             if (this%VF(ii,jj,kk).lt.VFlo)cycle
      !             if ((i.eq.ii).and.(j.eq.jj).and.(k.eq.kk)) cycle
      !             do ind=0,getSize(this%triangle_moments_storage(ii,jj,kk))-1
      ...
      !          if (surface_area .gt. 0.0_WP) then
      !             ! Only get projected SA for the polygons in the cell
      !             do n = 1, size_loc
      !                do nn = 1, size_adj
      !                   if (n.eq.nn) cycle
      !                   dot_result = dot_product(normals_adj(n,:),normals_adj(nn,:))
      ...
      !             surface_area = 0.0_WP
      !             do n = 1, size_loc
      !                this%norm_pos(i,j,k)=this%norm_pos(i,j,k)+norm_pos_loc(n)*area_adj(n)
      !                this%norm_neg(i,j,k)=this%norm_neg(i,j,k)+norm_neg_loc(n)*area_adj(n) 
      !                surface_area=surface_area+area_adj(n)
      !             end do
      !             this%norm_pos(i,j,k)=this%norm_pos(i,j,k)/surface_area
      !             this%norm_neg(i,j,k)=this%norm_neg(i,j,k)/surface_area
      !             ! this%norm_abs(i,j,k)=this%norm_pos(i,j,k)+this%norm_neg(i,j,k)
      !             ! this%norm_sig(i,j,k)=this%norm_pos(i,j,k)-this%norm_neg(i,j,k)
      !          end if
      !          deallocate(area_adj,normals_adj,norm_pos_loc,norm_neg_loc)
      !       end do 
      !    end do 
      ! end do
      ...
      this%norm_pos=tmp;this%norm_neg=tmp1; deallocate(tmp,tmp1)
      this%norm_abs=this%norm_pos+this%norm_neg
      this%norm_sig=this%norm_pos-this%norm_neg
      ! ! Get various measurements of the new metric
      ! do k=this%cfg%kmin_,this%cfg%kmax_
      !    do j=this%cfg%jmin_,this%cfg%jmax_
      !       do i=this%cfg%imin_,this%cfg%imax_
      ...
      !          if (this%VF(i,j,k).lt.VFlo.or.this%VF(i,j,k).gt.VFhi) cycle
      !          ! Count the number of triangles
      !          surface_area=0.0_WP; surface_norm=0.0_WP; size_dynamic = 0
      !          do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
      !             if (this%VF(ii,jj,kk).lt.VFlo)cycle
      !             size_dynamic = size_dynamic + getSize(this%triangle_moments_storage(ii,jj,kk))
      !          end do; end do; end do

      !          allocate(normals_dynamic(1:size_dynamic,1:3)); normals_dynamic = 0.0_WP
      !          allocate(area_dynamic   (1:size_dynamic));     area_dynamic    = 0.0_WP
      !          allocate(polyposnorm_dynamic(1:size_dynamic));polyposnorm_dynamic = 0.0_WP
      !          allocate(polynegnorm_dynamic(1:size_dynamic));polynegnorm_dynamic = 0.0_WP
      !          size_dynamic = 0
      !          ! Get surface area and normals of each triangles
      !          do kk=k-1,k+1; do jj=j-1,j+1; do ii=i-1,i+1
      !             if (this%VF(ii,jj,kk).lt.VFlo)cycle
      !             do ind=0,getSize(this%triangle_moments_storage(ii,jj,kk))-1
      !                call getMoments(this%triangle_moments_storage(ii,jj,kk),ind,volume_moments_and_normal)
      !                size_dynamic = size_dynamic+1
      !                area_dynamic(size_dynamic)      = getVolume(volume_moments_and_normal)
      !                normals_dynamic(size_dynamic,:) = normalize(getNormal(volume_moments_and_normal))
      !             end do
      !          end do; end do; end do
      !          surface_area = sum(area_dynamic)
      !          if (surface_area.gt.0.0_WP) then 
      !             surf_dot_pos_sum = 0.0_WP; surf_dot_neg_sum = 0.0_WP
      !             ! Get the postive and negative projected surface area
      !             do n = 1, size_dynamic
      !                do nn = 1, size_dynamic
      !                   if (n.eq.nn) cycle
      !                   dot_result = dot_product(normals_dynamic(n,:),normals_dynamic(nn,:))
      !                   if (dot_result .ge. 0.0_WP) polyposnorm_dynamic(n) = polyposnorm_dynamic(n) + area_dynamic(nn)*dot_result
      !                   if (dot_result .lt. 0.0_WP) polynegnorm_dynamic(n) = polynegnorm_dynamic(n) + area_dynamic(nn)*abs(dot_result)
      !                end do 
      !                polyposnorm_dynamic(n) = polyposnorm_dynamic(n)/(surface_area-area_dynamic(n))
      !                polynegnorm_dynamic(n) = polynegnorm_dynamic(n)/(surface_area-area_dynamic(n))
      !             end do
      !             ! Get the norms based on surface area weighting of the projected surface area
      !             do n = 1, size_dynamic
      !                surf_dot_pos_sum = surf_dot_pos_sum + polyposnorm_dynamic(n)*area_dynamic(n)
      !                surf_dot_neg_sum = surf_dot_neg_sum + polynegnorm_dynamic(n)*area_dynamic(n)
      !             end do
      !             this%norm_pos(i,j,k) = surf_dot_pos_sum/(surface_area)
      !             this%norm_neg(i,j,k) = surf_dot_neg_sum/(surface_area)
      !             this%norm_abs(i,j,k) = this%norm_pos(i,j,k) + this%norm_neg(i,j,k)
      !             this%norm_sig(i,j,k) = this%norm_pos(i,j,k) - this%norm_neg(i,j,k)
      !          end if
      !          deallocate(normals_dynamic,area_dynamic,polyposnorm_dynamic,polynegnorm_dynamic)
      !       end do
      !    end do
      ! end do
      ...
      !             end do
      !          end do; end do; end do
      !          ! this%SA_prior_reconst(i,j,k) = surface_area
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               ...
               if (this%norm_sig(i,j,k).ge. 0.5_WP .or. ((this%norm_sig(i,j,k).lt.0.5_WP) .and.(this%norm_abs(i,j,k).lt.0.75_WP))) then
   ...
   subroutine build_r2pnet(this)
      ...
      this%norm_abs = norm_pos + norm_neg
      this%norm_sig = norm_pos - norm_neg
      ! Traverse domain and reconstruct interface
