...
contains
   ...
   subroutine init(this)
      ...
      initialize_cfg: block
         ...
         call this%input%read('Lx',this%Lx); call this%input%read('nx',nx); allocate(x(nx+1))
         call this%input%read('Ly',this%Ly); call this%input%read('ny',ny); allocate(y(ny+1))
         call this%input%read('Lz',this%Lz); call this%input%read('nz',nz); allocate(z(nz+1))
         ! Create simple rectilinear grid in x y and z
         do i=1,nx+1
            x(i)=real(i-1,WP)/real(nx,WP)*this%Lx-0.5_WP*this%Lx
         end do
         do j=1,ny+1
            y(j)=real(j-1,WP)/real(ny,WP)*this%Ly-0.5_WP*this%Ly
         end do
         do k=1,nz+1
            z(k)=real(k-1,WP)/real(nz,WP)*this%Lz-0.5_WP*this%Lz
         end do
      ...
      initialize_lss: block
         ...
         if (this%ls%cfg%amRoot) then
            ...
            object1: block
               ...
               type(sgrid) :: grid
               ! Object size
               Lx=0.1_WP; Ly=0.1_WP; Lz=this%Lz
               ...
               do i=1,nx+1
                  x(i)=real(i-1,WP)*dx-0.5_WP*Lx-1.05_WP
               end do
               do j=1,ny+1
                  y(j)=real(j-1,WP)*dx-0.5_WP*Ly-0.005_WP
               end do
               do k=1,nz+1
               ...
               do k=1,nz
                  do j=1,ny
                     do i=1,nx
                        !Check if particles are inside the circle
                        if ((i-(0.05/dx))**2 + (j-(0.05/dx))**2 <= (0.05/dx)**2) then 
                           ! Increment particle
                           np=np+1
            ...
            object2: block
               ...
               type(sgrid) :: grid
               ! Object size
               Lx=0.35_WP; Ly=0.02_WP; Lz=this%Lz
               ! Create simple rectilinear grid
               nx=int(Lx/dx)
               ny=int(Ly/dx)
               nz=int(Lz/dx)
               allocate(x(1:nx+1),y(1:ny+1),z(1:nz+1))
               do i=1,nx+1
                  x(i)=real(i-1,WP)*dx+0.05_WP-1.05_WP-dx
               end do
               do j=1,ny+1
                  y(j)=real(j-1,WP)*dx-0.5_WP*Ly-0.005_WP
               end do
               ...
               do k=1,nz
                  do j=1,ny
                     do i=1,nx
                        ...
                        ! Fix left side, to depth of horizon (3 particles deep)
                        if (this%ls%p(np)%pos(1).lt.this%ls%delta) then
                           this%ls%p(np)%id=-2
                        end if
