...
contains
   ...
   subroutine init(this)
      ...
      implicit none
      class(punchtest), intent(inout) :: this
      
      ! Read the input
      this%input=inputfile(amRoot=amRoot,filename='input_punchtest')
      ...
      initialize_cfg: block
         ...
         do j=1,ny+1
            y(j)=real(j-1,WP)/real(ny,WP)*Ly-Ly-0.0005_WP
         end do
         do k=1,nz+1
      ...
      initialize_lss: block
         ...
         !call this%input%read('Lx',Lx);
         !call this%input%read('Ly',Ly);
         !call this%input%read('Lz',Lz);
         Lx=0.004_WP; Ly=0.002_WP; Lz=0.001_WP
         ...
         call this%input%read('Fracture Toughness',ft)

         this%ls%crit_energy = ft**2 / this%ls%elastic_modulus
         
         !this%ls%elastic_modulus=3.0e6_WP
         !this%ls%poisson_ratio=0.48_WP
            ...
            do k=1,nz
               do j=1,ny
                  do i=1,nx
                     ...
                     if (this%ls%p(np)%pos(2).gt.-1.0_WP*dx.and.abs(this%ls%p(np)%pos(1)).lt.0.001_WP) then
                        this%ls%p(np)%id=-1
                        this%ls%p(np)%vel=[0.0_WP,-0.5_WP,0.0_WP]
                     end if
                     !if (this%ls%p(np)%pos(2).lt.-9.0_WP*dx) then
                     !   this%ls%p(np)%id=-2
                     !end if
                     if (this%ls%p(np)%pos(2).lt.15.0_WP*dx.and.abs(this%ls%p(np)%pos(1)).lt.0.001_WP) then
                        this%ls%p(np)%id=-1
                        this%ls%p(np)%vel=[0.0_WP,-0.5_WP,0.0_WP]
                     end if
                  end do
      ...
      create_ensight: block
         ...
         this%ens_out=ensight(cfg=this%cfg,name='punchtest')
      ...
      create_monitor: block
         ...
         this%mfile=monitor(this%cfg%amRoot,'punchtest')
