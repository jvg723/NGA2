...
contains
   ...
   function levelset_jet(xyz,t) result(G)
      ...
      ! G =radius*(1+0.1*cos(2*3.14/0.1*xyz(2))) - sqrt((xyz(1)-center(1))**2 + (xyz(3)-center(3))**2)
   ...
   subroutine simulation_init
      ...
      print *, '1'
         ...
         !    ens_out=ensight(cfg=cfg,name='Jet_VE')
         !    print *, '1.01'
         ...
         do while (time%it.le.time%itmax)
            ...
            dirichlet_velocity: block
              ...
              do n=1,mybc%itr%no_
                 i=mybc%itr%map(1,n); j=mybc%itr%map(2,n); k=mybc%itr%map(3,n)
                 myr=sqrt(fs%cfg%xm(i)**2+fs%cfg%zm(k)**2)/radius
               !   fs%V(i,j,k)=min(0.0_WP,-2.0_WP*Ujet*(1.0_WP-myr**2)*(1.0_WP+0.2_WP*SIN(2.4_WP*time%t)))
                 fs%V(i,j,k)=min(0.0_WP,-Ujet*(1.0_WP+0.2_WP*SIN(2.4_WP*time%t)))
              end do
