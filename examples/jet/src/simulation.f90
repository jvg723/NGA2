...
contains
   ...
   subroutine simulation_init
      ...
      coupler_prep: block
         ...
         ! Block 1 to block 2
         cpl12x=coupler(src_grp=group,dst_grp=group,name='pipe_to_jet_x'); call cpl12x%set_src(cfg1,'x'); call cpl12x%set_dst(cfg2,'x'); call cpl12x%initialize()
         cpl12y=coupler(src_grp=group,dst_grp=group,name='pipe_to_jet_y'); call cpl12y%set_src(cfg1,'y'); call cpl12y%set_dst(cfg2,'y'); call cpl12y%initialize()
         cpl12z=coupler(src_grp=group,dst_grp=group,name='pipe_to_jet_z'); call cpl12z%set_src(cfg1,'z'); call cpl12z%set_dst(cfg2,'z'); call cpl12z%initialize()
