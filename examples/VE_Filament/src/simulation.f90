...
contains
   ...
   function levelset_filament(xyz,t) result(G)
      implicit none
      ...
      ! G=-radius+sqrt((xyz(1)-center(1))**2 + (xyz(3)-center(3))**2)
      G =radius*(1.0_WP+0.1_WP*cos(2_WP*3.14_WP/0.1_WP*xyz(2))) - sqrt((xyz(1)-center(1))**2 + (xyz(3)-center(3))**2)
   end function levelset_filament
   ...
   subroutine simulation_init
      ...
      create_and_initialize_vof: block
         ...
         ! Initialize a bubble
         call param_read('Filament position',center)
         ! call param_read('Bubble volume',radius)
         call param_read('Filament radius',radius)
         ...
         do k=vf%cfg%kmino_,vf%cfg%kmaxo_
            do j=vf%cfg%jmino_,vf%cfg%jmaxo_
               do i=vf%cfg%imino_,vf%cfg%imaxo_
                  ...
                  call cube_refine_vol(cube_vertex,vol,area,v_cent,a_cent,levelset_filament,0.0_WP,amr_ref_lvl)
      ...
      create_and_initialize_flow_solver: block
         ...
         use tpns_class, only: bcond,slip,wall,clipped_neumann,neumann,dirichlet
         use mathtools,  only: Pi
         ...
         ! call fs%add_bcond(name='bc_yp',type=slip,canCorrect=.true.,locator=yp_locator)
         !Slip at the bottom
         ! call fs%add_bcond(name='bc_ym',type=slip,face='y',dir=-1,canCorrect=.false.,locator=ym_locator)
         call fs%add_bcond(name='bc_xm',type=clipped_neumann,face='x',dir=-1,canCorrect=.false.,locator=xm_locator)
         call fs%add_bcond(name='bc_zm',type=clipped_neumann,face='z',dir=-1,canCorrect=.false.,locator=zm_locator)
      ...
      create_viscoelastic: block
         use tpviscoelastic_class, only: fenecr
         integer :: i,j,k
            ...
            do j=cfg%jmino_,cfg%jmaxo_
               do i=cfg%imino_,cfg%imaxo_
                  if (vf%VF(i,j,k).gt.0.0_WP) then
                     ve%SC(i,j,k,1)=0.0_WP !< Cxx
                     ve%SC(i,j,k,4)=0.0_WP !< Cyy
                     ve%SC(i,j,k,6)=0.0_WP !< Czz
                  end if
               end do
            end do
         end do
      ...
      create_ensight: block
         ...
         ens_out=ensight(cfg=cfg,name='VE_Filament')
