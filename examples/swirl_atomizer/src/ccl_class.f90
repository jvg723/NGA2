...
module ccl_class
   ...
   type :: meta_struct_type
      ...
      real(WP) :: average_radius
      real(WP) :: type
      integer  :: nnode
      real(WP) :: f_detached,f_ligament,f_sheet
      real(WP) :: min_thickness
      real(WP) :: xL,yL,zL,length
   ...
   type :: struct
      ...
      ! integer, dimension(:) :: films                      !< List of constituitive films
      integer :: type                                     !< Structure type; 0: droplet, 1: ligament, 2: sheet
      real(WP) :: min_thickness                           !< Global minimum film thickness
      integer :: minloc_rank                              !< Rank of global minimum film thickness
   end type struct
   ...
   type, extends(struct) :: film
      ...
      ! integer :: struct                                   !< For liquid film, ID of encompassing structure
      logical :: has_fragmented                           !< Has the film undergone breakup?
   end type film
   ...
   type :: ccl
      ...
      real(WP) :: thickness_cutoff=0.8_WP                    !< Maximum film thickness as fraction of meshsize
      real(WP) :: struct_thickness_cutoff=2.0_WP             !< Maximum struct thickness as "multiple" of meshsize
      logical  :: use_struct_cutoff=.false.                  !< Use struct_thickness_cutoff
      ...
      real(WP),dimension(:,:,:), allocatable :: film_edge1     !< Some measure of whether the film cell is an edge cell 
      real(WP),dimension(:,:,:,:), allocatable :: film_edge_normal !< Outward facing normal of film edge 
      integer, dimension(:,:,:), allocatable :: struct_type    !< Local struct type - 0: droplet, 1: ligament, 2: sheet
      real(WP),dimension(:,:,:), allocatable :: struct_thickness !< Local thickness of the struct cell
   ...
   contains
      ...
      procedure, private :: calculate_struct_thickness
      ...
      procedure :: meta_structures_stats
      procedure :: film_classify
      procedure :: struct_classify
      procedure :: get_min_thickness
      procedure :: get_min_struct_thickness      
      procedure :: sort_by_thickness
      procedure :: sort_struct_by_thickness
...
contains
   ...
   function constructor(cfg,name) result(self)
      ...
      allocate(self%film_edge1(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%film_edge1=0.0_WP
      ...
      ! Allocate struct type array
      allocate(self%struct_type(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%struct_type=0
      
      ! Allocate struct thickness array
      allocate(self%struct_thickness(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%struct_thickness=0.0_WP
   ...
   subroutine build_lists(this,VF,poly,U,V,W)
      ...
      if (present(poly)) then
         ...
         this%max_interface_planes=size(this%poly,1)
   ...
   subroutine label(this)
      ...
      ! Reset struct thickness
      this%struct_thickness=0.0_WP
      ...
      max_structs = ceiling(0.1_WP*npp/2.0_WP)
         ...
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  my_struct_thickness=this%calculate_struct_thickness(i,j,k)
                  ! Find untagged point in liquid phase
                  if (this%VF(i,j,k).ge.this%VFlo) then
                     if ((.not.this%use_struct_cutoff).or.(my_struct_thickness.lt.this%struct_thickness_cutoff*this%cfg%meshsize(i,j,k))) then
         ...
         do j= this%cfg%jmin_,this%cfg%jmax_
            do k= this%cfg%kmin_,this%cfg%kmax_
               if (this%VF(this%cfg%imax_,j,k).ge.this%VFlo .and. this%film_phase(this%cfg%imax_,j,k).eq.0) then
                  if ((.not.this%use_struct_cutoff).or.this%calculate_struct_thickness(this%cfg%imax_,j,k).lt.this%struct_thickness_cutoff*this%cfg%meshsize(this%cfg%imax_,j,k)) then
         ...
         do i= this%cfg%imin_,this%cfg%imax_
            do k= this%cfg%kmin_,this%cfg%kmax_
               if (this%VF(i,this%cfg%jmax_,k).ge.this%VFlo .and. this%film_phase(i,this%cfg%jmax_,k).eq.0) then
                  if ((.not.this%use_struct_cutoff).or.this%calculate_struct_thickness(i,this%cfg%jmax_,k).lt.this%struct_thickness_cutoff*this%cfg%meshsize(i,this%cfg%jmax_,k)) then
         ...
         do i= this%cfg%imin_,this%cfg%imax_
            do j= this%cfg%jmin_,this%cfg%jmax_
               if (this%VF(i,j,this%cfg%kmax_).ge.this%VFlo .and. this%film_phase(i,j,this%cfg%kmax_).eq.0) then
                  if ((.not.this%use_struct_cutoff).or.this%calculate_struct_thickness(i,j,this%cfg%kmax_).lt.this%struct_thickness_cutoff*this%cfg%meshsize(i,j,this%cfg%kmax_)) then                  
         ...
         do k=this%cfg%kmin_,this%cfg%kmax_
            do j=this%cfg%jmin_,this%cfg%jmax_
               do i=this%cfg%imin_,this%cfg%imax_
                  ...
                  ! Find untagged point in liquid phase
                  if (this%VF(i,j,k).ge.this%VFlo) then
                     if ((.not.this%use_struct_cutoff).or.this%calculate_struct_thickness(i,j,k).lt.this%struct_thickness_cutoff*this%cfg%meshsize(i,j,k)) then
                        do dim = 1,3
                           pos = 0
                           pos(dim) = -1
                           ii = i + pos(1)
      ...
      ! Maximum number of local structures or parts of structures across all procs
   ...
   ! Calculate struct thickness
   function calculate_struct_thickness(this,i,j,k) result(local_thickness)
      implicit none
      class(ccl), intent(inout) :: this
      integer, intent(in) :: i,j,k
      real(WP) :: local_thickness
      ...
      local_thickness = 2.0_WP*VOF_local_sum/(SD_local_sum+epsilon(1.0_WP))
      this%struct_thickness(i,j,k) = local_thickness
      return
   end function calculate_struct_thickness
   ...
   subroutine film_sync(this)
      ...
      film_phase: block
         ...
         integer :: id,m
         integer, dimension(1:this%cfg%nproc*this%n_film_max) :: phase_,phase

         ! Negative phase -> liquid, positive phase -> gas, defaults to gas in tie (this is easily changed)
         phase_=0
         do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film
            ...
            do n=1,this%film_list(this%film_map_(m))%nnode ! Loops over cells within local film segment
               ...
               phase_(id)=phase_(id)+2*this%film_phase(i,j,k)-3
            end do
         end do
         call MPI_ALLREDUCE(phase_,phase,this%cfg%nproc*this%n_film_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film ! Loops over film segments contained locally
            id=this%film_list(this%film_map_(m))%parent
            this%film_list(this%film_map_(m))%phase=(sign(1,phase(id))+3)/2
            do n=1,this%film_list(this%film_map_(m))%nnode ! Loops over cells within local film segment
               i=this%film_list(this%film_map_(m))%node(1,n)
               j=this%film_list(this%film_map_(m))%node(2,n)
               k=this%film_list(this%film_map_(m))%node(3,n)
               this%film_phase(i,j,k)=this%film_list(this%film_map_(m))%phase
            end do
         end do
      end block film_phase      
   ...
   subroutine meta_structures_sort(this)
      ...
      if (this%n_struct_max.eq.0) this%n_meta_struct=0
   ...
   subroutine meta_structures_stats(this,U,V,W)
      ...
      real(WP), dimension(:), pointer :: radius_,radius
      real(WP), dimension(:), pointer :: min_thickness_,min_thickness
      real(WP), dimension(:), pointer :: x_min_,x_min,x_max_,x_max
      real(WP), dimension(:), pointer :: y_min_,y_min,y_max_,y_max
      real(WP), dimension(:), pointer :: z_min_,z_min,z_max_,z_max
      ...
      allocate(radius(1:this%n_meta_struct),radius_(1:this%n_meta_struct))
      allocate(min_thickness(1:this%n_meta_struct),min_thickness_(1:this%n_meta_struct))
      allocate(x_min(1:this%n_meta_struct),x_min_(1:this%n_meta_struct))
      allocate(x_max(1:this%n_meta_struct),x_max_(1:this%n_meta_struct))
      allocate(y_min(1:this%n_meta_struct),y_min_(1:this%n_meta_struct))
      allocate(y_max(1:this%n_meta_struct),y_max_(1:this%n_meta_struct))
      allocate(z_min(1:this%n_meta_struct),z_min_(1:this%n_meta_struct))
      allocate(z_max(1:this%n_meta_struct),z_max_(1:this%n_meta_struct))
      ...
      radius(:) = 0.0_WP;radius_(:)= 0.0_WP
      min_thickness(:) = 0.0_WP;min_thickness_(:)= huge(1.0_WP)
      x_min(:) = 0.0_WP;x_min_(:)= huge(1.0_WP)
      x_max(:) = 0.0_WP;x_max_(:)= -huge(1.0_WP)
      y_min(:) = 0.0_WP;y_min_(:)= huge(1.0_WP)
      y_max(:) = 0.0_WP;y_max_(:)= -huge(1.0_WP)
      z_min(:) = 0.0_WP;z_min_(:)= huge(1.0_WP)
      z_max(:) = 0.0_WP;z_max_(:)= -huge(1.0_WP)
         ...
         do while(associated(this%my_struct))
            if (this%my_struct%id.eq.this%meta_structures(i)) then
               ...
               do j = 1,this%my_struct%nnode
                  ...
                  radius_(i) = radius_(i) + norm2([xtmp,ytmp,ztmp],dim=1)*this%cfg%vol(ii,jj,kk)*this%VF(ii,jj,kk)

                  ! Extents
                  if (this%max_interface_planes.eq.0) then
                     x_min_(i) = min(x_min_(i),this%cfg%xm(ii))
                     x_max_(i) = max(x_max_(i),this%cfg%xm(ii))
                     y_min_(i) = min(y_min_(i),this%cfg%ym(jj))
                     y_max_(i) = max(y_max_(i),this%cfg%ym(jj))
                     z_min_(i) = min(z_min_(i),this%cfg%zm(kk))
                     z_max_(i) = max(z_max_(i),this%cfg%zm(kk))
                  else
                     do n=1,this%max_interface_planes
                        if (getNumberOfVertices(this%poly(n,ii,jj,kk)).gt.0) then
                           d = calculateCentroid(this%poly(n,ii,jj,kk))
                           x_min_(i) = min(x_min_(i),d(1))
                           x_max_(i) = max(x_max_(i),d(1))
                           y_min_(i) = min(y_min_(i),d(2))
                           y_max_(i) = max(y_max_(i),d(2))
                           z_min_(i) = min(z_min_(i),d(3))
                           z_max_(i) = max(z_max_(i),d(3))
                        end if
                     end do
                  end if
      ...
      ! Sum volume-weighted distances from center of gravity
      call MPI_ALLREDUCE(radius_,radius,this%n_meta_struct,MPI_REAL_WP,MPI_SUM,this%cfg%comm,ierr)

      ! Get extents
      call MPI_ALLREDUCE(x_min_,x_min,this%n_meta_struct,MPI_REAL_WP,MPI_MIN,this%cfg%comm,ierr)
      call MPI_ALLREDUCE(x_max_,x_max,this%n_meta_struct,MPI_REAL_WP,MPI_MAX,this%cfg%comm,ierr)
      call MPI_ALLREDUCE(y_min_,y_min,this%n_meta_struct,MPI_REAL_WP,MPI_MIN,this%cfg%comm,ierr)
      call MPI_ALLREDUCE(y_max_,y_max,this%n_meta_struct,MPI_REAL_WP,MPI_MAX,this%cfg%comm,ierr)
      call MPI_ALLREDUCE(z_min_,z_min,this%n_meta_struct,MPI_REAL_WP,MPI_MIN,this%cfg%comm,ierr)
      call MPI_ALLREDUCE(z_max_,z_max,this%n_meta_struct,MPI_REAL_WP,MPI_MAX,this%cfg%comm,ierr)

      ! Get min thickness
      call MPI_ALLREDUCE(min_thickness_,min_thickness,this%n_meta_struct,MPI_REAL_WP,MPI_MIN,this%cfg%comm,ierr)
      ...
      this%meta_structures_list(:)%vol_=0.0_WP
      ...
      do i=1,this%n_meta_struct
         ...
         this%meta_structures_list(i)%vol_= vol_struct_(i)
         ...
         this%meta_structures_list(i)%average_radius= radius(i)/vol_struct(i)
         this%meta_structures_list(i)%min_thickness= min_thickness(i)
         this%meta_structures_list(i)%xL = x_max(i)-x_min(i)
         this%meta_structures_list(i)%yL = y_max(i)-y_min(i)
         this%meta_structures_list(i)%zL = z_max(i)-z_min(i)
         this%meta_structures_list(i)%length = hypot(hypot(this%meta_structures_list(i)%xL,this%meta_structures_list(i)%yL)**2,this%meta_structures_list(i)%zL)
         ...
         this%meta_structures_list(i)%length = max(this%meta_structures_list(i)%length,this%meta_structures_list(i)%lengths(1))
   ...
   subroutine film_classify(this,Lbary,Gbary)
      ...
      integer, parameter :: nneigh = 1
      real(WP) :: xtmp, ytmp, ztmp
      real(WP) :: x_vol,y_vol,z_vol,vol_total
      real(WP) :: gx_vol,gy_vol,gz_vol,gvol_total
      ...
      real(WP), dimension(3) :: gc_local, gc_filter
      ...
      do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film ! Loops over film segments contained locally
         ...
         if (this%film_list(this%film_map_(m))%phase.eq.1) then ! Liquid film
            do n=1,this%film_list(this%film_map_(m))%nnode ! Loops over cells within local film segment
               ...
               j = this%film_list(this%film_map_(m))%node(2,n)
               k = this%film_list(this%film_map_(m))%node(3,n)
               vol_total = 0.0_WP; gvol_total = 0.0_WP
               x_vol = 0.0_WP; y_vol = 0.0_WP; z_vol = 0.0_WP
               gx_vol = 0.0_WP; gy_vol = 0.0_WP; gz_vol = 0.0_WP
               Imom(:,:) = 0.0_WP
               do kk = k-nneigh,k+nneigh
                  do jj = j-nneigh,j+nneigh
                     do ii = i-nneigh,i+nneigh
                        ...
                        ! Volume
                        gvol_total = gvol_total + this%cfg%vol(ii,jj,kk)*(1.0_WP-this%VF(ii,jj,kk))

                        ! Center of gravity
                        gx_vol = gx_vol + xtmp*this%cfg%vol(ii,jj,kk)*(1.0_WP-this%VF(ii,jj,kk))
                        gy_vol = gy_vol + ytmp*this%cfg%vol(ii,jj,kk)*(1.0_WP-this%VF(ii,jj,kk))
                        gz_vol = gz_vol + ztmp*this%cfg%vol(ii,jj,kk)*(1.0_WP-this%VF(ii,jj,kk))
               ...
               this%film_edge(i,j,k) = d(2)/(d(1)+tiny(1.0_WP))
               this%film_edge1(i,j,k) = d(2)/(sqrt(d(1)*d(3))+tiny(1.0_WP))

               ! Calculate edge normal
               c_local = Lbary(:,i,j,k)
               c_filter = [x_vol,y_vol,z_vol]/vol_total
               gc_local = Gbary(:,i,j,k)
               gc_filter = [gx_vol,gy_vol,gz_vol]/gvol_total
               this%film_edge_normal(:,i,j,k) = (c_local-c_filter)/(norm2(c_local-c_filter)+tiny(1.0_WP))
               ! this%film_edge(i,j,k) = norm2(c_filter-gc_filter)
            end do
         else ! Gas film
      ...
      ! Decide on one film type for each film
      film_type: block
         use mpi_f08,  only: MPI_ALLREDUCE,MPI_SUM,MPI_INTEGER
         integer :: id,ierr
         integer, dimension(1:this%cfg%nproc*this%n_film_max,0:2) :: type_,type
         ...
         do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film
            ...
            do n=1,this%film_list(this%film_map_(m))%nnode ! Loops over cells within local film segment
               ...
               j=this%film_list(this%film_map_(m))%node(2,n)
               k=this%film_list(this%film_map_(m))%node(3,n)
               type_(id,this%film_type(i,j,k))=type_(id,this%film_type(i,j,k))+1
            end do
         end do
         call MPI_ALLREDUCE(type_(:,0),type(:,0),this%cfg%nproc*this%n_film_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         call MPI_ALLREDUCE(type_(:,1),type(:,1),this%cfg%nproc*this%n_film_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         call MPI_ALLREDUCE(type_(:,2),type(:,2),this%cfg%nproc*this%n_film_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film ! Loops over film segments contained locally
            id=this%film_list(this%film_map_(m))%parent
            this%film_list(this%film_map_(m))%type=maxloc(type(id,:),dim=1)-1
            do n=1,this%film_list(this%film_map_(m))%nnode ! Loops over cells within local film segment
               i=this%film_list(this%film_map_(m))%node(1,n)
               j=this%film_list(this%film_map_(m))%node(2,n)
               k=this%film_list(this%film_map_(m))%node(3,n)
      ...
      call this%cfg%sync(this%film_edge1)
      call this%cfg%sync(this%film_edge_normal)

   end subroutine film_classify
   

   !> Classify struct by shape
   subroutine struct_classify(this,Lbary,Gbary)
      implicit none
      class(ccl), intent(inout) :: this
      real(WP), dimension(:,this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in) :: Lbary  !< Liquid barycenter
      real(WP), dimension(:,this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(in) :: Gbary  !< Gas barycenter
      ...
      this%struct_type = 0
      do m=this%sync_offset+1,this%sync_offset+this%n_struct ! Loops over struct segments contained locally
         do n=1,this%struct_list(this%struct_map_(m))%nnode ! Loops over cells within local struct segment
            i = this%struct_list(this%struct_map_(m))%node(1,n)
            j = this%struct_list(this%struct_map_(m))%node(2,n)
            k = this%struct_list(this%struct_map_(m))%node(3,n)
            vol_total = 0.0_WP; gvol_total = 0.0_WP
            x_vol = 0.0_WP; y_vol = 0.0_WP; z_vol = 0.0_WP
            gx_vol = 0.0_WP; gy_vol = 0.0_WP; gz_vol = 0.0_WP
            Imom(:,:) = 0.0_WP
            ...
            ! l2 = sqrt(5.0_WP/2.0_WP*abs(d(3)+d(1)-d(2))/vol_total)
            ! l3 = sqrt(5.0_WP/2.0_WP*abs(d(1)+d(2)-d(3))/vol_total)
            this%struct_type(i,j,k) = 0
            if (d(3).gt.(ratio*d(1))) this%struct_type(i,j,k) = this%struct_type(i,j,k) + 1
            if (d(3).gt.(ratio*d(2))) this%struct_type(i,j,k) = this%struct_type(i,j,k) + 1
            ! if (l1.gt.(ratio*l2)) struct_type(i,j,k) = struct_type(i,j,k) + 1
            ! if (l1.gt.(ratio*l3)) struct_type(i,j,k) = struct_type(i,j,k) + 1
         end do
      end do

      ! Decide on one struct type for each struct
      choose_type: block
         use mpi_f08,  only: MPI_ALLREDUCE,MPI_SUM,MPI_INTEGER
         integer :: id,ierr
         integer, dimension(1:this%cfg%nproc*this%n_struct_max,0:2) :: type_,type

         type_=0
         do m=this%sync_offset+1,this%sync_offset+this%n_struct
            id=this%struct_list(this%struct_map_(m))%parent
            do n=1,this%struct_list(this%struct_map_(m))%nnode ! Loops over cells within local struct segment
               i=this%struct_list(this%struct_map_(m))%node(1,n)
               j=this%struct_list(this%struct_map_(m))%node(2,n)
               k=this%struct_list(this%struct_map_(m))%node(3,n)
               type_(id,this%struct_type(i,j,k))=type_(id,this%struct_type(i,j,k))+1
            end do
         end do
         call MPI_ALLREDUCE(type_(:,0),type(:,0),this%cfg%nproc*this%n_struct_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         call MPI_ALLREDUCE(type_(:,1),type(:,1),this%cfg%nproc*this%n_struct_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         call MPI_ALLREDUCE(type_(:,2),type(:,2),this%cfg%nproc*this%n_struct_max,MPI_INTEGER,MPI_SUM,this%cfg%comm,ierr)
         do m=this%sync_offset+1,this%sync_offset+this%n_struct ! Loops over struct segments contained locally
            id=this%struct_list(this%struct_map_(m))%parent
            this%struct_list(this%struct_map_(m))%type=maxloc(type(id,:),dim=1)-1
         end do
         ! Also update meta_structures - need to use global type array
         do m=1,this%n_meta_struct
            this%meta_structures_list(m)%type=maxloc(type(this%meta_structures_list(m)%id,:),dim=1)-1
            this%meta_structures_list(m)%nnode=sum(type(this%meta_structures_list(m)%id,:),dim=1)
            this%meta_structures_list(m)%f_detached=real(type(this%meta_structures_list(m)%id,0),WP)/real(this%meta_structures_list(m)%nnode,WP)
            this%meta_structures_list(m)%f_ligament=real(type(this%meta_structures_list(m)%id,1),WP)/real(this%meta_structures_list(m)%nnode,WP)
            this%meta_structures_list(m)%f_sheet=real(type(this%meta_structures_list(m)%id,2),WP)/real(this%meta_structures_list(m)%nnode,WP)
         end do
      end block choose_type      

      ! Update ghost cells
      call this%cfg%sync(this%struct_type)

   end subroutine struct_classify
   
   !> Find the minimum thickness of each film and the processor in which the minimum thickness cell is located
   subroutine get_min_thickness(this)
      use mpi_f08,  only: MPI_ALLREDUCE,MPI_MINLOC
      use parallel, only: MPI_2REAL_WP
      implicit none
      class(ccl), intent(inout) :: this
      integer  :: id,m,n,i,j,k,ierr
      real(WP), dimension(1:2,1:this%cfg%nproc*this%n_film_max) :: min_thickness_,min_thickness

      if (this%n_film_max.eq.0) return ! If there are no films globally
      min_thickness_(1,:)=huge(1.0_WP); min_thickness_(2,:)=this%cfg%rank
      do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film ! Loops over film segments contained locally
         id=this%film_list(this%film_map_(m))%parent
         do n=1,this%film_list(this%film_map_(m))%nnode ! Loops over cells within local film segment
            i=this%film_list(this%film_map_(m))%node(1,n)
            j=this%film_list(this%film_map_(m))%node(2,n)
            k=this%film_list(this%film_map_(m))%node(3,n)
            min_thickness_(1,id)=min(min_thickness_(1,id),this%film_thickness(i,j,k))
         end do
      end do
      call MPI_ALLREDUCE(min_thickness_,min_thickness,this%cfg%nproc*this%n_film_max,MPI_2REAL_WP,MPI_MINLOC,this%cfg%comm,ierr)
      do m=this%film_sync_offset+1,this%film_sync_offset+this%n_film ! Loops over film segments contained locally
         id=this%film_list(this%film_map_(m))%parent
         this%film_list(this%film_map_(m))%min_thickness = min_thickness(1,id)
         this%film_list(this%film_map_(m))%minloc_rank   = min_thickness(2,id)
      end do
   end subroutine get_min_thickness

   !> Find the minimum thickness
   subroutine get_min_struct_thickness(this)
      use mpi_f08,  only: MPI_ALLREDUCE,MPI_MIN
      use parallel, only: MPI_REAL_WP
      implicit none
      class(ccl), intent(inout) :: this
      integer  :: id,m,n,i,j,k,ierr
      real(WP), dimension(1:this%cfg%nproc*this%n_struct_max) :: min_thickness_,min_thickness

      if (this%n_struct_max.eq.0) return ! If there are no films globally
      min_thickness_=huge(1.0_WP)
      do m=this%sync_offset+1,this%sync_offset+this%n_struct ! Loops over film segments contained locally
         id=this%struct_list(this%struct_map_(m))%parent
         do n=1,this%struct_list(this%struct_map_(m))%nnode ! Loops over cells within local film segment
            i=this%struct_list(this%struct_map_(m))%node(1,n)
            j=this%struct_list(this%struct_map_(m))%node(2,n)
            k=this%struct_list(this%struct_map_(m))%node(3,n)
            min_thickness_(id)=min(min_thickness_(id),this%struct_thickness(i,j,k))
         end do
      end do
      call MPI_ALLREDUCE(min_thickness_,min_thickness,this%cfg%nproc*this%n_struct_max,MPI_REAL_WP,MPI_MIN,this%cfg%comm,ierr)
      do m=this%sync_offset+1,this%sync_offset+this%n_struct ! Loops over film segments contained locally
         id=this%struct_list(this%struct_map_(m))%parent
         this%struct_list(this%struct_map_(m))%min_thickness = min_thickness(id)
      end do
      ! Also update meta_structures - need to use global type array
      do m=1,this%n_meta_struct
         this%meta_structures_list(m)%min_thickness=min_thickness(this%meta_structures_list(m)%id)
      end do
   end subroutine get_min_struct_thickness   
   ...
   !> Sort struct indices by increasing thickness
   subroutine sort_struct_by_thickness(this)
      implicit none
      class(ccl), intent(inout) :: this
      integer  :: m,n
      real(WP), dimension(  :), allocatable :: mythick
      ! Nothing to do if there is no local struct
      if (this%n_struct.eq.0) return
      ! Build an auxiliary array with thickness for sorting for each struct
      do m=this%sync_offset+1,this%sync_offset+this%n_struct
         ! Allocate and copy 1D thickness and node map
         allocate(mythick(    this%struct_list(this%struct_map_(m))%nnode))
         do n=1,this%struct_list(this%struct_map_(m))%nnode
            mythick(n)=this%struct_thickness(this%struct_list(this%struct_map_(m))%node(1,n),&
            &                              this%struct_list(this%struct_map_(m))%node(2,n),&
            &                              this%struct_list(this%struct_map_(m))%node(3,n))
         end do
         ! Sort it
         call quick_sort_by_thickness(mythick,this%struct_list(this%struct_map_(m))%node)
         ! Deallocate thickness and map
         deallocate(mythick)
      end do
   contains
   ...
   end subroutine sort_struct_by_thickness   
