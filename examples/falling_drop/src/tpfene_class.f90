...
module tpfene_class
   ...
   !> Constant density tpfene solver object definition
   type, extends(tpscalar) :: tpfene
      ! Model parameters
   ...
   contains
      procedure :: tpfene_initialize                       !< Initialization two phase fene class
      ...
      procedure :: get_max=>tpfene_get_max                 !< Augment tpscalar's default monitoring
   end type tpfene
...
contains
   ...
   subroutine tpfene_initialize(this,cfg,model,name)
      implicit none
      class(tpfene) :: this
      class(config), target, intent(in) :: cfg
      integer, intent(in) :: model
      character(len=*), optional :: name
      ! Create a six-scalar solver for conformation tensor (all scalars in liquid)
      ...
      allocate(this%visc_p(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%visc_p=0.0_WP
      allocate(this%SRmag (this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_)); this%SRmag =0.0_WP
   end subroutine tpfene_initialize
   
   
   !> Compute visc_p from SR using Carreau model
   subroutine update_visc_p(this,SR)
      implicit none
      class(tpfene), intent(inout) :: this
   ...
   subroutine tpfene_get_max(this,VF)
      ...
      deallocate(tmp)
   end subroutine tpfene_get_max

end module tpfene_class