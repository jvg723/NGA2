...
module tpns_class
    ...
    contains
      ...
      !  procedure :: hole_puncturing
      !  procedure :: hole_expanding
 ...
 contains
    ...
    subroutine add_slipvel(this,vf,ss,Uslip,Vslip,Wslip,Hfilm)
       ...
       real(WP), dimension(:,:), allocatable:: Loc_,Loc,EdgeN_,EdgeN!,Loc_p,EdgeN_p,Am
       real(WP), dimension(:), allocatable:: fcoeff !fcoeff_p,FilmThick_,FilmThick,bm,IPIV
       ...
       a=0.4_WP; b=0.3_WP; dx=maxval(this%cfg%dx); Nedge_ = 0; threshold = 0.3_WP; 
       ...
       do k=this%cfg%kmin_,this%cfg%kmax_
          do j=this%cfg%jmin_,this%cfg%jmax_
             do i=this%cfg%imin_,this%cfg%imax_
                if (vf%edge_sensor(i,j,k).gt.threshold.and.vf%thin_sensor(i,j,k).eq.1.0_WP.and.vf%VF(i,j,k).ge.VFlo.and.vf%VF(i,j,k).lt.VFhi) then   
                  Nedge_ = Nedge_+1
                end if
             end do
          end do
       ...
       if (Nedge .gt. 0) then 
          allocate(Loc_(3,Nedge_))
          allocate(EdgeN_(3,Nedge_))
         !  allocate(FilmThick_(Nedge_))
             ...
             do j=this%cfg%jmin_,this%cfg%jmax_
                do i=this%cfg%imin_,this%cfg%imax_
                  if (vf%edge_sensor(i,j,k).gt.threshold .and. vf%thin_sensor(i,j,k).eq.1.0_WP.and.vf%VF(i,j,k).ge.VFlo.and.vf%VF(i,j,k).lt.VFhi) then   
                     ...
                     ! print *, 'location',this%cfg%xm(i),this%cfg%ym(j),this%cfg%zm(k),'and edge sensor', vf%edge_sensor(i,j,k),"thin_sensor",vf%thin_sensor(i,j,k),"this VF",vf%VF(i,j,k), "This is VFhi", VFhi
                     ! print *, "Here is the plane ",real(getNumberOfPlanes(vf%liquid_gas_interface(i,j,k)),WP)
                     ! print *, "THis is the plane info:", getPlane(vf%liquid_gas_interface(i,j,k),0)
                     !  FilmThick_(Nedge_) = vf%thickness(i,j,k)
                   end if
                end do
             end do
          end do
          ...
          allocate(fcoeff(Nedge))
         !  allocate(FilmThick(Nedge))
          count = 0
         ...
         !  call MPI_ALLGATHERV(FilmThick_,Nedge_,MPI_REAL_WP,FilmThick,Nedge_list,dispels,MPI_REAL_WP,this%cfg%comm)
 
          ! Linear system of equations to solve for coefficients of each stokeslets
          ! allocate(Am(Nedge,Nedge))
          ! allocate(bm(Nedge))
          ! allocate(IPIV(Nedge))
          ! do i =1, Nedge
          !    do j =1, Nedge
          !       pos = (Loc(:,i)-Loc(:,j))/xscale
          !       ftemp = EdgeN(:,j)
          !       ftemp = stokeslet_force()
          !       Am(i,j) = dot_product(ftemp,EdgeN(:,i))
          !    end do 
          ! end do
          ! bm = Utc
          ! call DGESV(Nedge,1,Am,Nedge,IPIV,bm,Nedge,info)
          ! if (info.eq.0) then
          !    fcoeff = bm
          ! else
          !    ! can't inversely solve the system
          !    fcoeff = Utc
          ! end if
          ! deallocate(Am,bm,IPIV)
 
          ! Some other approximation based on placing the point source at the liquid barycenter
          ! fcoeff = 0.0_WP
          ! do i =1, Nedge
          !    do j =1, Nedge
          !       pos = (Loc(:,i)-Loc(:,j))/xscale
          !       r = norm2(pos)
          !       if (r .lt. 6.0_WP) then 
          !          fcoeff(i) =  fcoeff(i) + 1.0_WP
          !       end if
          !    end do 
          !    fcoeff(i) = Utc/(fcoeff(i))
          ! end do
          
          ! Evaluate the retraction based on individual film thickness
          !  do i = 1,Nedge
          !     fcoeff(i) = sqrt(2.0_WP*this%sigma/(this%rho_l*FilmThick(i)))
          !  end do
          fcoeff = Utc
         ...
         !  if (vf%cfg%amRoot) then
          ...
          do n = 1, Nedge_
             ss%p(n)%pos = Loc_(:,n)
             ss%p(n)%fcoeff = Utc!fcoeff(n)
             ss%p(n)%nedge = EdgeN_(:,n)
             ss%p(n)%VF = 0.0_WP
          end do
         !  end if
 
          ! Deal with periodic conditions for thin films
          ! allocate(Loc_p(3,Nedge*9))
          ! allocate(EdgeN_p(3,Nedge*9))
          ! allocate(fcoeff_p(Nedge*9))
          ! ! Provide 9 copies of stokeslet sources
          ! do i =1,3
          !    do j =1,3
          !       do k = 1,Nedge
          !          ind = ((3*(i-1) + j)-1)*Nedge + k
          !          EdgeN_p(:,ind) = EdgeN(:,k)
          !          Loc_p(1,ind) = Loc(1,k)+(i-2)*this%cfg%xL
          !          Loc_p(3,ind) = Loc(3,k)+(j-2)*this%cfg%zL
          !          Loc_p(2,ind) = Loc(2,k)
          !          fcoeff_p(ind) = fcoeff(k)
          !       end do
          !    end do
          ! end do
         ...
         !  deallocate(FilmThick_,FilmThick)
          ! deallocate(Loc_p, EdgeN_p,fcoeff_p)
         else
            ss%np_=0;ss%np = 0
            call ss%resize(0)
       end if
   ...
   !  subroutine hole_puncturing(this,vf,punctured)
   !     use vfs_class, only: vfs
   !     class(tpns),      intent(inout) :: this
   !     class(vfs),       intent(inout) :: vf
   !     logical, intent(inout) :: punctured
   !     integer :: i,j,k,ii,jj,kk,range
   !     if (punctured) then
   !     else
   !        i=32;j=32;k=61;range=1
   !        do kk = -range,range
   !           do jj = -range,range
   !              do ii = -range,range
   !                 vf%VF(i+ii,j+jj,k+kk) = 0.0_WP
   !              end do
   !           end do
   !        end do
   !        ! vf%VF(i,j,k)=0.0_WP
   ...
   !  end subroutine hole_puncturing
 
 
   !  subroutine hole_expanding(this,vf,dt,Uslip,Vslip,Wslip,Hfilm,h_int_counter)
   !     use vfs_class, only: vfs
   !     use mpi_f08
   !     use parallel,   only: MPI_REAL_WP
   !     use mathtools, only: normalize
   ...
   !     real(WP), intent(in) :: dt,Hfilm
   !     integer, intent(inout) :: h_int_counter
   !     real(WP), dimension(3) :: Loc,ftemp,pos
   !     real(WP) :: xscale,dx,Utc,r
   !     integer :: i,j,k,ii,jj,kk,range
   !     dx=maxval(this%cfg%dx); xscale=1.0_WP*dx
   !     Utc = sqrt(2.0_WP*this%sigma/(this%rho_l*2.0_WP*Hfilm))
 
   !     ! do k=this%cfg%kmin_,this%cfg%kmax_
   !     !   do j=this%cfg%jmin_,this%cfg%jmax_
   !     !      do i=this%cfg%imin_,this%cfg%imax_
   !     !         if (i.eq.32.and.j.eq.32.and.k.eq.61)then
   !     !          do kk = -range,range
   !     !             do jj = -range,range
   !     !                do ii = -range,range
   !     !                   ! vf%VF(i+ii,j+jj,k+kk) = 0.0_WP
   !     !                   call setNumberOfPlanes(vf%liquid_gas_interface(i+ii,j+jj,k+kk),1)
   !     !                   call setPlane(vf%liquid_gas_interface(i+ii,j+jj,k+kk),0,[0.0_WP,0.0_WP,0.0_WP],-1.0_WP) 
   !     !                end do
   !     !             end do
   !     !          end do
   !     !       end if
   ...
   !     Loc = [vf%cfg%xm(32),vf%cfg%ym(32),vf%cfg%zm(61)]
   !     do k=this%cfg%kmino_,this%cfg%kmaxo_
   !        do j=this%cfg%jmino_,this%cfg%jmaxo_
   !           do i=this%cfg%imino_,this%cfg%imaxo_  
                
   !              pos = ([this%cfg%x(i),this%cfg%ym(j),this%cfg%zm(k)]-Loc)/xscale
   !              r = norm2(pos)
   !              ftemp = normalize(pos)
   !              ! Uslip(i,j,k)=Uslip(i,j,k)+Utc*ftemp(1)/(r**3)
   !              Uslip(i,j,k) = Utc*ftemp(1)/(r**2)
   !              ! if (r.lt.5.0_WP) then
   !              !    Uslip(i,j,k) = dx/dt!(pos(1)/r)*dx/dt
   !              ! end if
 
   !              pos = ([this%cfg%xm(i),this%cfg%y(j),this%cfg%zm(k)]-Loc)/xscale
   !              r = norm2(pos)
   !              ftemp = normalize(pos)
   !              ! Vslip(i,j,k)=Vslip(i,j,k)+Utc*ftemp(2)/(r**3)
   !              Vslip(i,j,k) = Utc*ftemp(2)/(r**2)
   !              ! if (r.lt.5.0_WP) then
   !                 ! Vslip(i,j,k) = dx/dt!(pos(2)/r)*dx/dt
   !              ! end if
 
   !              pos = ([this%cfg%xm(i),this%cfg%ym(j),this%cfg%z(k)]-Loc)/xscale
   !              r = norm2(pos)
   !              ftemp = normalize(pos)
   !              Wslip(i,j,k) = Utc*ftemp(3)/(r**2)
   !              ! Wslip(i,j,k)=Wslip(i,j,k)++Utc*ftemp(3)/(r**3)
   !              ! if (r.lt.5.0_WP) then
   !                 ! Wslip(i,j,k) = dx/dt!(pos(3)/r)*dx/dt
   !              ! end if
   !           end do
   !        end do
   !     end do
       
   !     ! call MPI_AllGATHER(myloc,3,MPI_REAL_WP,Loc,1,MPI_INTEGER,this%cfg%comm,ierr)
 
   !     h_int_counter = h_int_counter+1
   !     ! h_int_counter = 10
       
   !  end subroutine hole_expanding
 
    ! !Using possion solvers to solve for a divergence free velocity field
    ! subroutine add_slipvel(this,vf,ss,dt,Uslip,Vslip,Wslip,Hfilm)
    !    use stokeslet_class, only: stokeslet
    !    use vfs_class, only: vfs
    !    implicit none
    !    class(tpns),      intent(inout) :: this
    ...
    !    real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Wslip !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    !    real(WP), intent(in) :: dt,Hfilm
    !    real(WP) :: threshold, TCvel
    !    integer :: i,j,k
    !    Uslip = 0.0_WP; Vslip = 0.0_WP; Wslip = 0.0_WP
    !    threshold = 0.3_WP; TCvel = sqrt(2.0_WP*this%sigma/(this%rho_l*2.0_WP*Hfilm))
    !    ! Simple interpolation of edge velocities to each face
    !    do k=this%cfg%kmino_,this%cfg%kmaxo_
    !       do j=this%cfg%jmino_,this%cfg%jmaxo_
    !          do i=this%cfg%imino_,this%cfg%imaxo_
    !             if (vf%edge_sensor(i,j,k).gt.threshold) then
    !                Uslip(i  ,j,k) = Uslip(i  ,j,k) + 0.5_WP*vf%edge_normal(1,i,j,k)*TCvel
    !                Uslip(i+1,j,k) = Uslip(i+1,j,k) + 0.5_WP*vf%edge_normal(1,i,j,k)*TCvel
    !                Vslip(i,j  ,k) = Vslip(i,j  ,k) + 0.5_WP*vf%edge_normal(2,i,j,k)*TCvel
    !                Vslip(i,j+1,k) = Vslip(i,j+1,k) + 0.5_WP*vf%edge_normal(2,i,j,k)*TCvel
    !                Wslip(i,j,k  ) = Wslip(i,j,k  ) + 0.5_WP*vf%edge_normal(3,i,j,k)*TCvel
    !                Wslip(i,j,k+1) = Wslip(i,j,k+1) + 0.5_WP*vf%edge_normal(3,i,j,k)*TCvel
    !             end if
    !          end do
    !       end do
    !    end do
    !    call this%makedivfree(Uslip,Vslip,Wslip)
    ! end subroutine add_slipvel
    ...
    !    real(WP), intent(in) :: dt,Hfilm
    !    integer, dimension(:), allocatable :: Nedge_list,dispels
    !    real(WP), dimension(:,:), allocatable:: myLoc,myEdgeN,Loc,EdgeN,Loc_p,EdgeN_p,Am
    !    real(WP), dimension(:), allocatable:: fcoeff,fcoeff_p,bm,IPIV
    !    real(WP), dimension(3) :: ftemp,pos,c1,c2
    !    real(WP) :: a,b,dx,xscale,epsilon,Utc,r,threshold
    !    integer :: myNedge,Nedge,count,info,i,j,k,ind,ierr,n
    !    Uslip = 0.0_WP; Vslip = 0.0_WP; Wslip = 0.0_WP
    !    a=0.3_WP; b=0.3_WP; dx=maxval(this%cfg%dx); myNedge = 0; threshold = 0.8_WP; 
    !    xscale=a*dx; epsilon=b*dx
    !    Utc = sqrt(2.0_WP*this%sigma/(this%rho_l*2.0_WP*Hfilm))
    ...
    !                   myNedge = myNedge+1
    !                   ! myLoc(:,myNedge) = vf%Lbary(:,i,j,k)
    !                   myLoc(:,myNedge) = [this%cfg%xm(i),this%cfg%ym(j),this%cfg%zm(k)]
    ...
    !          fcoeff(i) = Utc/(fcoeff(i))
    !       end do
    !       ss%np_ = 0; ss%np = Nedge
    !       if (vf%cfg%amRoot) then
    !          ss%np_ = Nedge
    !          call ss%resize(Nedge)
    !          do n = 1, Nedge
    !             ss%p(n)%pos = Loc(:,n)
    !             ss%p(n)%fcoeff = fcoeff(n)
    !             ss%p(n)%nedge = EdgeN(:,n)
    !             ss%p(n)%VF = 0.0_WP
    !          end do
    !       end if
    ...
    !          end do
    !       end do
    !       deallocate(myLoc,myEdgeN,dispels,Loc,EdgeN,fcoeff)
    !       ! deallocate(Loc_p, EdgeN_p,fcoeff_p)
    ...
    ! Regular possion solver for making slip velocity divergence free
    subroutine makedivfree(this,Uslip,Vslip,Wslip)
       implicit none
       class(tpns),      intent(inout) :: this
       real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Uslip
       real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Vslip
       real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: Wslip
       real(WP), dimension(:,:,:) , allocatable :: Ucorrect,Vcorrect,Wcorrect
       integer :: i,j,k
       allocate(Ucorrect(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
       allocate(Vcorrect(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
       allocate(Wcorrect(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_))
       do k=this%cfg%kmin_,this%cfg%kmax_+1
          do j=this%cfg%jmin_,this%cfg%jmax_+1
             do i=this%cfg%imin_,this%cfg%imax_+1
                this%div(i,j,k)=sum(this%divp_x(:,i,j,k)*Uslip(i:i+1,j,k))+&
                &               sum(this%divp_y(:,i,j,k)*Vslip(i,j:j+1,k))+&
                &               sum(this%divp_z(:,i,j,k)*Wslip(i,j,k:k+1))
             end do
          end do
       end do
       do k=this%cfg%kmin_,this%cfg%kmax_+1
       ...
       call this%psolv%setup()
       this%psolv%rhs = -this%cfg%vol*this%div
       this%psolv%sol = 0.0_WP
       call this%psolv%solve()
       call this%shift_p(this%psolv%sol)
       this%dPjx=0.0_WP;this%dPjy=0.0_WP;this%dPjz=0.0_WP
       call this%get_pgrad(this%psolv%sol,Ucorrect,Vcorrect,Wcorrect)
       Uslip=Uslip-Ucorrect
       Vslip=Vslip-Vcorrect
       Wslip=Wslip-Wcorrect
       ...
       ! call this%get_max()
       ! if (this%cfg%amRoot) print *, 'The max divergence after this operation is: ', this%divmax
       deallocate(Ucorrect,Vcorrect,Wcorrect)
    end subroutine makedivfree
