...
contains
   ...
   subroutine add_surface_tension_jump_thin(this,dt,div,vf)
      ...
      class(vfs), intent(inout) :: vf
      integer :: i,j,k
      real(WP), dimension(2) :: alpha
      ! real(WP), dimension(2) :: curv
      ! Store old jump
      this%DPjx=this%Pjx
      this%DPjy=this%Pjy
      this%DPjz=this%Pjz
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ! X face ===========================================
               alpha=0.0_WP
               call vf%get_twoalpha(i,j,k,'x',alpha)
               this%Pjx(i,j,k)=this%sigma*sum(this%divu_x(:,i,j,k)*alpha)
               ! this%Pjx(i,j,k)=this%sigma*curv(1)*sum(this%divu_x(:,i,j,k)*VF(:,1))&
               ! &              +this%sigma*curv(2)*sum(this%divu_x(:,i,j,k)*VF(:,2))
               ! Y face ===========================================
               alpha=0.0_WP
               call vf%get_twoalpha(i,j,k,'y',alpha)
               this%Pjy(i,j,k)=this%sigma*sum(this%divv_y(:,i,j,k)*alpha)
               ! this%Pjy(i,j,k)=this%sigma*curv(1)*sum(this%divv_y(:,i,j,k)*VF(:,1))&
               ! &              +this%sigma*curv(2)*sum(this%divv_y(:,i,j,k)*VF(:,2))
               ! Z face ===========================================
               alpha=0.0_WP
               call vf%get_twoalpha(i,j,k,'z',alpha)
               this%Pjz(i,j,k)=this%sigma*sum(this%divw_z(:,i,j,k)*alpha)
               ! this%Pjz(i,j,k)=this%sigma*curv(1)*sum(this%divw_z(:,i,j,k)*VF(:,1))&
               ! &              +this%sigma*curv(2)*sum(this%divw_z(:,i,j,k)*VF(:,2))
   ...
   ! subroutine add_surface_tension_jump_thin(this,dt,div,vf)!,SFX,SFY,SFZ,SFXn,SFYn,SFZn,t)
   !    use messager,  only: die
   !    use vfs_class, only: vfs
   !    use irl_fortran_interface
   !    implicit none
   ...
   !    ! real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SFXn  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
   !    ! real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SFYn  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
   !    ! real(WP), dimension(this%cfg%imino_:,this%cfg%jmino_:,this%cfg%kmino_:), intent(inout) :: SFZn !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
   !    ! integer, intent(in) :: t
   !    class(vfs), intent(inout) :: vf
   !    integer :: i,j,k
   !    real(WP), dimension(2,2) :: sub_VF
   !    real(WP), dimension(2) :: curv_interp
   !    ! Store old jump
   !    this%DPjx=this%Pjx
   !    this%DPjy=this%Pjy
   !    this%DPjz=this%Pjz
   ...
   !          do i=this%cfg%imin_,this%cfg%imax_+1
   !             ! X face ===========================================
   !             sub_VF=0.0_WP; curv_interp=0.0_WP
   !             call vf%get_curvFace(i,j,k,'x',sub_VF,curv_interp)
   !             this%Pjx(i,j,k)=this%sigma*curv_interp(1)*sum(this%divu_x(:,i,j,k)*sub_VF(:,1))&
   !             &              +this%sigma*curv_interp(2)*sum(this%divu_x(:,i,j,k)*sub_VF(:,2))
   !             ! Y face ===========================================
   !             sub_VF=0.0_WP; curv_interp=0.0_WP
   !             call vf%get_curvFace(i,j,k,'y',sub_VF,curv_interp)
   !             this%Pjy(i,j,k)=this%sigma*curv_interp(1)*sum(this%divv_y(:,i,j,k)*sub_VF(:,1))&
   !             &              +this%sigma*curv_interp(2)*sum(this%divv_y(:,i,j,k)*sub_VF(:,2))
   !             ! Z face ===========================================
   !             sub_VF=0.0_WP; curv_interp=0.0_WP
   !             call vf%get_curvFace(i,j,k,'z',sub_VF,curv_interp)
   !             this%Pjz(i,j,k)=this%sigma*curv_interp(1)*sum(this%divw_z(:,i,j,k)*sub_VF(:,1))&
   !             &              +this%sigma*curv_interp(2)*sum(this%divw_z(:,i,j,k)*sub_VF(:,2))
   ...
   !       end do
   !    end do
   !    ! SFXn=abs(this%Pjx-SFX)
   !    ! SFYn=abs(this%Pjy-SFY)
   !    ! SFZn=abs(this%Pjz-SFZ)
   ! end subroutine add_surface_tension_jump_thin
