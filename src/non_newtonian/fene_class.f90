...
contains
   ...
   subroutine addsrc_PTT(this,gradU,resSC)
      ...
      ! Allocate rate of deformation tensor components
	   allocate(D(this%cfg%imino_:this%cfg%imaxo_,this%cfg%jmino_:this%cfg%jmaxo_,this%cfg%kmino_:this%cfg%kmaxo_,6))
      
      ! Rate of deformation tensor D=(1/2)(gradU+gradU^T)
      do k=this%cfg%kmino_,this%cfg%kmaxo_
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_
               ! Skip non-solved cells
               if (this%mask(i,j,k).ne.0) cycle
               D(i,j,k,1)=gradU(1,1,i,j,k)
               D(i,j,k,2)=0.5_WP*(gradU(2,1,i,j,k)+gradU(1,2,i,j,k))
               D(i,j,k,3)=0.5_WP*(gradU(3,1,i,j,k)+gradU(1,3,i,j,k))
               D(i,j,k,4)=gradU(2,2,i,j,k)
               D(i,j,k,5)=0.5_WP*(gradU(3,2,i,j,k)+gradU(2,3,i,j,k))
               D(i,j,k,6)=gradU(3,3,i,j,k)
            end do
         end do
      end do
